...
impl<'a, C: AsRef<[u8]>> Iterator for NamesIter<'a, C> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.index < self.count {
            ...
            match read_u64(&mut cursor).map_or(Err(FloreumError::Truncation), |ok| {
                ok.try_into()
                    .ok()
                ...
                Ok(len) => Some(
                    cursor
                        .get(..len)
                        .map_or(Err(FloreumError::Truncation), |ok| {
...
impl<C: AsRef<[u8]> + for<'a> From<&'a [u8]>> Names<C> {
    pub fn from_bytes(bytes: &mut &[u8]) -> Result<Self, FloreumError> {
        ...
        let count = read_u64(bytes)?
            .try_into()
            .map_err(|_| FloreumError::LocalBitWidth)?;
